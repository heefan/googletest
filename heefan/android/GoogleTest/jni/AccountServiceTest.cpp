...
{
private:
	std::map<std::string, Account> mAccount; // an internal map to store all Accounts for test
...
TEST(AccountServiceTest, transferTest)
{
	...
	// always invoke AccountHelper::findAccountForUser when AccountManager::findAccountForUser is invoked
	EXPECT_CALL(*pManager, findAccountForUser(testing::_)).WillRepeatedly(
		testing::Invoke(&helper, &AccountHelper::findAccountForUser));

	// always invoke AccountHelper::updateAccount when AccountManager::updateAccount is invoked
	...
	// check the balance of Account("A") and Account("B") to verify that AccountService has done the right job
